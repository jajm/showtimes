...
async function getTheatersWithin (latitude, longitude, distanceInMeters) {
    ...
    const theatersWithDistance = theaters.map(t => {
        return Object.assign({}, t, { distance: distance(latitude, longitude, t.latitude, t.longitude) });
    });
    const theatersWithinDistance = theatersWithDistance.filter(t => t.distance < distanceInMeters);

    return theatersWithinDistance;
}
